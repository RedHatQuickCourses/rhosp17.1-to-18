...
metadata:
  name: openstack-nova-compute-ffu-cell1
spec:
  nodeSets:
    - compute0-1-set
  servicesOverride:
...
[source,bash,role=execute]
----
${BASH_ALIASES[openstack]} server --os-compute-api-version 2.48 show --diagnostics server-2-compute-1 2>&1 || echo FAIL
${BASH_ALIASES[openstack]} server --os-compute-api-version 2.48 show --diagnostics server-1-compute-0 2>&1 || echo FAIL
----
