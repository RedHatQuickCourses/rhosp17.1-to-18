...
[source,bash,role=execute,subs=attributes]
----
cat /home/stack/templates/overcloud-vip-deployed.yaml
----
+
Sample output:
+
...
----
+
You can also get the internal_api VIP by accessing one of the controller nodes:
...
[source,bash,role=execute,subs=attributes]
----
ssh tripleo-admin@172.25.249.16
----

.. Query the internal_api VIP:
+
[source,bash,role=execute,subs=attributes]
----
...
.. Logout from the controller node:
+
...
----
+
Then the values for SOURCE_MARIADB_IP and CONTROLLER_IP are:
+
----
...
.Passwords file and credentials preparation

. On the *director host*, copy the overcloud passwords and SSH keys to the student user home directory:
+
[source,bash,role=execute,subs=attributes+]
----
...
. On the *workstation host*, copy the overcloud passwords and SSH keys from the director node to the utility node:
+
[source,bash,role=execute,subs=attributes+]
----
...
. On the *utility host*, extract the pull secret from OpenShift configuration:
+
[source,bash,role=execute,subs=attributes+]
----
...
. Authenticate to the Red Hat registry using the extracted credentials:
+
[source,bash,role=execute,subs=attributes+]
----
...
. Update the OpenShift pull secret with the authenticated credentials:
+
[source,bash,role=execute,subs=attributes+]
----
...
. On the *utility host*, export the shell variables for the following outputs and test the connection to the {OpenStackShort} database:
+
[source,bash,role=execute]
----
...
. Configure the utility host to access the control plane:
+
[source,bash,role=execute]
----
sudo -i
nmcli con delete c5ca8081-6db2-4602-4b46-d771f4330a6d
nmcli con delete 9096615b-bf89-34e5-8abc-f1b87533e849
nmcli con add type ethernet ifname eth3 con-name eth3-static \
  ipv4.addresses 172.25.249.253/24 \
