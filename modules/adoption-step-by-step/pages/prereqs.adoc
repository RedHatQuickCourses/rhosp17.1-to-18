...
. The lab environment will be ready in approximately 40-50 minutes. Execute the following command to check the status of the lab environment:
+
[source,bash,role=execute,subs=attributes]
----
sh wait.sh
...
The kube-apiserver operator is ready
Machine Config Operator changes applied
Lock file detected: /.etcd-migration-ocp4. Etcd migration already completed
Starting and enabling the status-api service to notify classroom API that OCP cluster is ready...
Service status-api started and enabled
...
----
Name                                              Phase
cert-manager-operator.v1.17.0           Succeeded
----
