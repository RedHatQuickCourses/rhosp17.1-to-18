...
. Retrieve the existing `auth_encryption_key` and `service` passwords. You use these passwords to patch the `osp-secret`. In the following example, the `auth_encryption_key` is used as `HeatAuthEncryptionKey` and the `service` password is used as `HeatPassword`, in the *utility server*:
+
[source,bash,role=execute]
----
HEAT_AUTH_ENCRYPTION_KEY_PASSWORD=$(cat ~/overcloud-passwords.yaml | grep ' HeatAuthEncryptionKey:' | awk -F ': ' '{ print $2; }')
